...
export default async function DashboardPage() {
  ...
  const getTipoIcon = (comp: ProximoCompromisso) => {
    if (comp.status === "concluido") {
      return <CheckCircle2 className="h-5 w-5 text-gray-400" />;
    }
    return comp.tipo === "vacinacao" ? (
  ...
  const formatarData = (dataStr: string, hora?: string) => {
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border shadow-sm">
          ...
          <div className="divide-y">
            {compromissosPendentes.length === 0 ? (
              ...
              compromissosPendentes.slice(0, 8).map((comp) => (
                <Link
                  key={comp.id}
                  href={`/animais/${comp.animalId}`}
                  className={`flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors ${
                    comp.status === "concluido" ? "opacity-60 bg-gray-50" : ""
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-medium truncate ${
                      comp.status === "concluido" ? "text-gray-500 line-through" : "text-gray-900"
                    }`}>
                      {comp.animalNome}
                  ...
                  <div className="flex-shrink-0 text-right">
                    {getUrgenciaBadge(comp)}
                    <p className="text-xs text-gray-400 mt-1">
                      {formatarData(comp.dataAgendada, comp.hora)}
                    </p>
                  </div>
                </Link>
              ))
